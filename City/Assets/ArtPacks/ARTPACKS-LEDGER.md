...
## lab-glass（13 件·本仓自焙·r140 P-39① 预置+r168 D-20260926-02 remedy 2u 档扩+r185 D-20260926-06 1u 档扩+r188 时态计数器条）——集团自产件
...
- 增量（r185·D-20260926-06 候选④ 1u 变体自焙线·T-FV-003 施工序第一段）：**1u 档两件自焙**=lab-birth-1u 24×72=1×3u（竖排诞生站：上玻璃舱+下 2×3 六卡槽出生墙两亮位+暗冷基座——2u 横排版的 1u 竖排原生重绘·滚动语义同归时态接线轮）+lab-sandbox-1u 24×48=1×2u（紧凑沙盘台：全息光带+两楼微缩城+最高楼天线+桌板双腿）——r169 全岸最大净跨 1.8333u<2u 证死 2u 槽·S1..S5 五枚 1u×3u 槽在证（D-06 优先双落=S5 集群贴邻+S4 贴塔侧）；**1u 件比例门=24px 宽@PPU24 原生重绘非缩放重采样**；确定性双跑 SHA 幂等（birth-1u 9047A1E755B0／sandbox-1u A1DEC543A73A）+存量 10 件 SHA 与 r140/r168 提交版逐位同+沙盒断言门 logs/devloop-r185-lab1u-test.ps1（A0 ASCII 审计/A1 盘 census 12+IHDR/A2 存量 10 对记录/A3 新件 SHA 钉+1u 宽度律+12 互异/A4 角透明/A5 独立 lit·glass 复算 172·320/12）全绿+风格闸双读 PASS（4× 台架+1× 实尺寸接触表多模态判：结构可辨/冷调统一零暖色零近白大板/1× 连贯剪影/零硬伤——弱读四面〔全息带 1× 偏暗/未亮槽对比低/舱体中调平/桌板下缘 1× 糊〕流向打磨池非阻塞）；.meta 随下个编辑器轮首导（P-27⑤·lab-glass→24 文件夹级行在册零新行）；布设沙盒轮（S5/S4 落位+席位门族零回归）待领。

- 消费实况（r187·P-39 施工序第三段引擎轮·labs-manifest v0.2 coupled_updates_r187·D-20260926-06 候选④）：**1u 双落入城（LabBirth01 [23,9,24,12] S5 集群贴邻+LabSandbox01 [-6,9,-5,11] S4 贴塔侧）**——lab-birth-1u 1+lab-sandbox-1u 1 消费（LabsRules 纯表 12→14·LabBirth/LabSandbox 精灵族 GO order 3·零 tilemap 笔触·城层 40/49/54+props 44 前后不动证）+LabsProof 3633 断言（双落面钉/立足胞派生/五窗渲染门/重载门 14/14）+6 pass 管线全绿（braincrown/facades labKept=14 计数过+camera 基线零漂+resident 雨落邻接零回归）+四截图 docs/design/m1-r187-labs1u-*.png；**.meta 双件本轮首导**（P-27⑤·lab-glass→24 文件夹级行在册零新行）；2u 档两件（r168）维持留盘未消费（2u 槽几何死面=r169 负定谳在册）；滚动卡/沙盘提案时态接线归 M2 时态轮（禁静态烤死数字·stale 诚实律）。
- 增量（r188·P-39 施工序第四段时态接线轮·T-FV-003 勘定+预置半）：**lab-digits 7-seg 计数器条 80×10 自焙**（十位 7×10 胞 pitch 8——诞生站人口读出/沙盘 open 提案读出的数字基件·@PPU24=每位 0.292×0.417u·运行时逐位切片 Sprite.Create〔r99 图集先例〕；ON 段=亮核 200,240,255 α235〔舱窗/亮槽家族色〕+OFF 段=淡显示栅 50,90,140 α70〔活设备读法非死像素〕；**值=运行时活读禁烤死数字**〔D-02 stale 诚实律·条带只载字形零数值〕）——确定性双跑 SHA 幂等（**A73F354E5F14**）+独立复算门 logs/devloop-r188-labdigits-test.ps1 17 断言全绿（不信任烘焙打印：全 70 段逐 px 复算对 7-seg 真值表/色律精确/10 胞互异〔1 vs 7 不对称证〕/gap 列透明/复跑幂等）+风格闸 4× 台架多模态 PASS（十位全可辨零误读/淡栅不糊字/零伪影/冷青全息家族读法）；**时态接线律正典=Tools/city/labs-temporal-manifest.json**（fluxverse-labstemporal/0.1·r189 引擎轮唯一参数源：诞生站=人口读出浮于管道带上方+出生墙六亮槽←census.wall 滚动〔wall[0] 最亮=最新卡〕+RESIDENT_BIRTH 仪式脉冲〔舱辉光+新槽爆闪·city_action birth_station_cabin_print〕/沙盘=open 提案读出+PROPOSAL_NEW 全息带闪+PROPOSAL_APPLIED 双微缩块亮〔registry city_action 两 refs·D-02 唯一活态面〕/孵化舱=待机律零脉冲〔LAB 三型 reserved·T-FV-104〕+10s 轮询+grandfather 诚实降级）+布设沙盒门 logs/devloop-r188-labstemporal-test.ps1 70 断言全绿（r186 解析律全承：双读出 cap backing 对 144 障碍全普查零命中/槽位 art 矩形对烘焙器 canon cols/rows 交叉/art→world 公式手钉复算/登记簿三型 emitting+LAB reserved+孵化舱零脉冲/四负控起火/活数据键 smoke）；.meta 随 r189 引擎轮首导（P-27⑤·lab-glass→24 文件夹级行在册零新行）。
- 消费实况（r189·P-39 施工序第五段时态接线引擎轮·labs-temporal-manifest coupled_updates_r189 照单·D-20260926-06 候选④）：**lab-digits 消费**=CityLabsTemporal 运行时逐位切片活值读出（诞生站人口信标 census.total〔当日 10003 五位〕+沙盘 open 提案〔当日 19 两位〕·10s 活读禁烤死数字·D-02）+出生墙六亮槽/仪式舱辉+新槽爆闪/全息带/双微缩块=LabTmp* 运行时叠加族（零盘残·r146）——**.meta 本轮首导**（P-27⑤·lab-glass→24 文件夹级行在册零新行）+LabsProof 4321（F0 镜像+F1 真值表逐 px+活值读出/墙律/脉冲三连/D-temporal 渲染门）+7 pass 管线全绿+scan/verify 双绿+多模态 10003/19 逐位零误读；夜帧小读出可辨性候选入打磨池（T-FV-109 勘定轮）。
...
- 增量（r203·00:10 美术整改令批2 T-FV-122 S1③·「水面中央黑色三角锥异物」=r202 勘定倒影暗化过度读作物）：**refl 四源重暗化 0.4→0.6**（bake-water-fx.ps1 `$dark`）——四件重源=refl-quant **EBF8D26A1B15**／refl-game **B674D68221E9**／refl-media **783F534691FD**／refl-tower **5EE3872684E0**（**溯源门四针新钉** 0.4 旧派生 D5E099FF1233/2A4F1867B936/098E58F50129/18E50A1808F2·r176 双针 C007613EFAE7/4BA113444F98 就此出役）+11 未触件 already-current（dark 作用面恰四件的机械证）+harness B 面独立复算（x0.6 翻转律抽样 mism=0·logs/devloop-r203-watertiles-test.ps1 22 断言组内）。

## water-tiles/（8 件·本仓自焙·r203 00:10 整改令批2 T-FV-122 S1① 水帞帧自焙）——集团自产件

- 内容：**蓝紫渐变+柔波相位 8 帧**（16×16·water_wave_00..07）——批2 令文①「水面是粉色砖块纹理非水面」根治件：蓝→紫竖向渐变基（44,52,118→52,58,130）+1/5 密度 hash 抖动+双柔波带（crest 96,108,196/under-edge 66,76,144·行 5.5/11.5 反相对位）+稀疏波峰辉点（158,170,230·帞分列帧动画）——**相位律=2px/frame×8 帧=16px 恰一帞宽**（动画无缝循环+帞缘平铺衔接的构造律：波函数吃 (x+phase+poff) mod 16 周期 16）；环境色通道（水面=环境色正典·紫粉禁入功能光同面律）；全帞不透明——「半透明」令文落 S2 tilemap renderer α 档（r202 预注册）非美术面。
- 落位：`Assets/Art/CleanCityv3/WaterTiles/`（**r202 草案「Assets/Art/WaterTiles」勘正**：ArtRoot 硬钉 CleanCityv3〔builder L25 实读〕→MK 零改律优先·S2 重指仅换 MK 路径串 Tiles_Extract→WaterTiles）；PPU=导入器默认 16 档（1u/帞·CleanCity 水帞同世界尺寸）**零 PpuFor 行**。
- 来源：本仓 GDI+ 程序化自焙（`Tools/city/bake-water-tiles.ps1`·ASCII 律·零外采零许可义务）。
- 验证门（r203）：烘焙器自检（波带计数 24..60/蓝族律逐像素 b>r+avg≥40/双跑 SHA/八帞互异/盘字节回验）+harness **22 断言全绿**——SHA12 族 D2D74DECF189/84E06A1172FD/7A41F7082B8A/E5144172F4B4/9EB18625C8ED/6748B670CAF9/D1CC849A56AB/73E1B95512B3+帧动 f0-f1 差 51px/f0-f4 差 87px+渐变律 row0b=118→row15b=130。
- 风格闸（接触表 logs/devloop-r203-watertiles-sheet.png 多模态五面）：**总 PASS**——旧 CleanCity 帞「粉紫错缝砖墙读」批评实锤确认/新八帞蓝紫水面+相位移动可辨/平铺 16×6 无缝零竖直断层/零粉红污染零近白大板/零豆腐零撕裂（等距波峰节奏=水面风格化可接受注记·旧帧 deprecated 随 S2 换装执行）。
- 消费实况：**引擎接线待 S2 水面引擎轮**（builder t_water_0..7 重指 WaterTiles+WaterFxRules v0.3〔玫瑰洗退役/c5 蓝紫门重定义/renderer α 0.85-0.95 档〕+WaterFxProof 扩容+水帧重拍）。
...
- 消费实况（r179 S5b 引擎落地）：**ResidentLabelsUI 适配器挂池**（City/Assets/Scripts/ResidentLabelsUI.cs·场景持久根 CityLabelsUI·uGUI ScreenSpaceOverlay 运行时 canvas=r16 批内不可渲染律·byte-path 32 池 census==32·L1 街景档 only=L0 零文字·≤6 同屏=屏幕距离排序 tie=slot·hidden 席零标签=r124 三件套律 UI 面继承）+**世界层 32 牌退役**（NameTag GO 清扫入 LabelProof·b1 旧牌 plate-res-b1-*.png 留盘退役面=P-21④ 义）+TagProof→ResidentLabelProof 转型（世界牌 census==0+UI 池 census==32+纯核金对+相机投影交叉核）+邻接证明 census 门族重锚（Resident/Labs/Office/Vehicle/Bubble/Card/Facade/BrainCrown 九面 32→0）；.meta 32 件随本轮首编辑器导入（P-27⑤·PpuFor resident-labels→100 行 r178 已备）。

## skyline-shanghai/（1 件·本仓自焙·r207 00:10 整改令批2 T-FV-122 S3 远景天际线条自焙）——集团自产件

- 内容：**far-shanghai.png 576×324 远景剪影条**（批2 令文②「远景天际线 3-5 栋可辨识上海轮廓楼+其余方格随机变化」根治件）——**五命名形内嵌**：pearl 双球塔〔小 orbs 上叠大 orb+柱+splayed 三腿+天线·cx130〕／twist 扭锥〔五节段收分+节段错位=扭转读法·cx260〕／step 阶梯尖顶〔setback 四段收分·cx380〕／bud 花苞〔球茎剖面+暗萼缝竖线+双叶簇·脑塔冠同 motif·cx470〕／mast 电台桅〔横桁双 crossbar+阶梯基座·cx510·兼供右缘漂移极窗覆盖〕+**hash 变奏 17 地块**（宽 10..46/隙 0..3·高塔 19% 顶行 63..79/中档 42% 77..92/低档 39% 94..129·antenna/水箱/peach 檐带/peach 窗条纹/slate 窗缝 toppings 按 hash 逐件）+基座质量块 rows 120..234（远方城市底质量·零透视漏洞）；谱=pack 原生 rose (240,147,161) 基色+peach (255,205,186) accents+slate (45,28,42) 稀疏窗缝——**FogFar 乘法雾律与 r34 验证面逐位同源**（rose kinship/purple lean/夜暗化证明族自动过）。
- **契约面（S4 引擎轮零几何 churn 的构造性保证）**：画布 576×324=SkylineRules SourceW/H 逐位·**内容顶行 61→world +19.0=FarContentTopY 逐位**（天窗律 19.04 下 0.04u 余量·尖顶行 61 恒地标独占=generic tops 钉 ≥63）·底行 234=r34 量测行界 → QuadCenterY/Covers/FarTopRow/FarContentTopY/FogFar 全常量零改·S4 重指面=CityAmbient 序列化 skylineFar+SkylineProof FarPath 两点。
- 落位：`Assets/ArtPacks/skyline-shanghai/`（**新包落位=parallax-skyline 8 件 PKG_COUNT 钉版的规避面**·ArtPacksStockProof census 零破）·PPU=导入器默认 16 档（parallax 族同世界尺寸·自然 36×20.25+×2 整缩 point 律）**零 PpuFor 行**·.meta 随 S4 首编辑器轮导入（P-27⑤·pre-editor 零 meta=诚实态）。
