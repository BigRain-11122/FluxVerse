...
## office-towers/（14 件·AA-016.02 裁切 11+本仓自焙 3·r151 P-38② 预置入仓）

- 内容：**hi-bit 立面双径资产**（r150 货源定谳 B/C 兑现）——①3×3 族直采 11 件 144×144px@PPU24=6×6u（MEDIA footprint 逐位精确=r150②）：tower-glass-31..37 玻璃方塔 7〔顶带+格栅基座·k37 暗板岩=r150「dusk 气质最强」MEDIA 换装首选〕+tower-mid-39..42 三开间中楼 4〔**k41=源表字节级精确重复 k39**·AA-016 表内同款两画位·DUP_OF 普查留痕 r149 pair-dup 同律〕；②微窗皮自焙 3 件：facade-quant 120×192=5×8u（金 accent）／facade-game 120×120=5×5u（青 accent）／facade-annex 48×72=2×3u（青 accent）——规则微窗格 2×3px+8px 楼层带+冠线 accent+暗基带·**窗灯中性面**（静态零亮窗=禁装饰律·点亮率归 P-38③ 热力线）。
- 来源：直采 11=**AA-016.02 S 库 L2 裁切衍生**（`Tools/city/crop-office-towers.ps1` 零重采样·源路径承 landmark-sheets.txt〔r53 律〕·源只读 size+mtime 双不动·紧框门 11/11 零 trim〔occ=1×10+0.914×1〕·双跑 SHA 稳·distinct 10/11〔源内精确对 1〕）——授权链承 AA-016（2026-09-22 用户全量确认购置合法+U121 轻档 M1/M2 研发期）零新许可义务；自焙 3=**本仓 GDI+**（`Tools/city/bake-facade-skin.ps1` r140 范式·双跑 SHA 稳 17DD5EB3AF73／B5307D95A363／D22B8E620D43·像素类普查门+未亮窗律门〔>80 亮度唯 accent 冠线〕+全不透明门）零外采零许可义务。
- 风格闸（r151 锚预演·r44 律三轴）：**三轮定谳**——首焙近黑底 FAIL（dusk tint 下棕调）→紫移 v1 仍 FAIL→根因律=**dusk tint α0.22 恒加 (56,35,24)=任何底 r≥56 硬底·近黑底结构性到不了锚紫族**→正解=皮面自带锚族基值（art-target-dusk 未亮楼体实测 bins 40,40,80／48,48,88／56,56,104 采样定色）→终判三轴全 PASS+**实境贴合绿**（皮贴现城 QUANT/GAME/ANNEX 塔位合成图）；C 组 11 件三轴同过（窗灯缺席=设计态·k31-37 中层空白=源件词汇·v1 观察池=竖向 mullion hint 候选）；预演证据=logs/devloop-r151-tower-preview.{py,png}。
- 消费实况（r151 预置）：**引擎布设待 r152 布设沙盒轮**（挂载矩阵 manifest：tile 上位 OfficeRules 族先例/MEDIA 立面换装 k37 直用候选/QUANT+GAME_ANNEX 皮面挂载·r137/r141 槽位门族全套）→r153 引擎轮（证明扩容+四截图 §三 复扫+r44 和谐度随证）；.meta 随首个编辑器轮导入生成（P-27⑤ 先例·CityImportPostprocessor PPU 表已增 office-towers→24 行）。
