# FluxVerseTick v1.0 - the city heartbeat (10-min OS loop, group-standard mechanism)
# ASCII-only script (encoding law). Chinese mandate lives in mandate.txt (UTF-8).
# Round: 1) run perceptor  2) run verify gate  3) log  4) rotate logs (7 days)
# Exit 0 = healthy round; 1 = gate FAIL (old world-state is kept by scan order).
