...
| ID | 任务 | 指针/判据 | 状态 |
|---|---|---|---|
| T-FV-002 | 00:10 美术整改令分轮施工：①核心 tile 集/上海天际线轮廓强化（禁蓝方块平铺）②水面 L0 显著度 ③黄昏黄金档验证 ④名牌迁 UI 壳层 ⑤与 art-target-dusk 并排对比图过 §五 6 条验收 | orders.md 09-26 00:10 行；TECH §九 r180 行（S2-S5 已收口 r175-r179〔三剪影+冠入城+名牌 UI 迁壳〕·S6a 参数定谳已收口 r180〔duskgold-manifest：窗灯率律 v2 BASE0.30+ramp+金带 α75+水暖/景深洗参数面〕·下一领=S6b 黄金档引擎轮：率律 v2 接线+金带 v0.3+水暖/景深洗挂载+AlbumShot dusk 两帧+四判据门→S7 并排图册）；判据=新截图肉眼接近目标图气质；品字 F-03/门债 F-10/hue-shift F-17 三呈报件随整改批再呈（D-20260926-05⑧） | open |
